...
with num_cond_col1:
    ...
    num_prenatal_visits = st.number_input("Number of prenatal visits:", min_value=0, format="%d") # New widget
...
with num_cond_col2:
    ...
    month_prenatal_care_began = st.number_input("Month prenatal care began:", min_value=1, max_value=12, format="%d") # New widget
...
if st.button("Submit Data"):
    data = {
        ...
        "Number of prenatal visits": num_prenatal_visits, # Added to data dictionary
        "Month prenatal care began": month_prenatal_care_began # Added to data dictionary
    }
    ...
    previs = data["Number of prenatal visits"]
    pmcarb_month = data["Month prenatal care began"]
    ...
    # Generate risk factors message
    risk_factors_list = []
    if data["Pre Pregnancy Diabetes"] == "Yes":
        risk_factors_list.append("Pre-Pregnancy Diabetes")
    if data["Gestational Diabetes"] == "Yes":
        risk_factors_list.append("Gestational Diabetes")
    if data["Pre Pregnancy Hypertension"] == "Yes":
        risk_factors_list.append("Pre-Pregnancy Hypertension")
    if data["Gestational Hypertension"] == "Yes":
        risk_factors_list.append("Gestational Hypertension")
    if data["Hypertension Eclampsia"] == "Yes":
        risk_factors_list.append("Hypertension Eclampsia")
    if data["Prior C-Section"] == "Yes":
        risk_factors_list.append("Prior C-Section")
    if data["Syphilis"] == "Yes":
        risk_factors_list.append("Syphilis")
    if data["Chlamydia"] == "Yes":
        risk_factors_list.append("Chlamydia")
    if data["Gonorrhea"] == "Yes":
        risk_factors_list.append("Gonorrhea")
    if data["Hepatitis B"] == "Yes":
        risk_factors_list.append("Hepatitis B")
    if data["Hepatitis C"] == "Yes":
        risk_factors_list.append("Hepatitis C")
    if data["Infertility Treatment Used"] == "Yes":
        risk_factors_list.append("Infertility Treatment Used")
    if data["Weight Gained (lb)"] > 0:
        risk_factors_list.append(f"Weight Gained ({data['Weight Gained (lb)']:.2f} lb)")
    if data["Cigarettes in Trimester 1"] > 0:
        risk_factors_list.append(f"Cigarettes in Trimester 1 ({data['Cigarettes in Trimester 1']} per day)")
    if data["Cigarettes in Trimester 2"] > 0:
        risk_factors_list.append(f"Cigarettes in Trimester 2 ({data['Cigarettes in Trimester 2']} per day)")
    if data["Cigarettes in Trimester 3"] > 0:
        risk_factors_list.append(f"Cigarettes in Trimester 3 ({data['Cigarettes in Trimester 3']} per day)")
    if data["Number of prenatal visits"] > 0:
        risk_factors_list.append(f"Number of prenatal visits ({data['Number of prenatal visits']})")
    if data["Month prenatal care began"] > 0:
        risk_factors_list.append(f"Month prenatal care began ({data['Month prenatal care began']})")

    if risk_factors_list:
        st.write("Your risk of needing a C-Section may be higher because:")
        for factor in risk_factors_list:
            st.write(f"- {factor}")
    else:
        st.write("Based on the provided information, there are no immediate high-risk factors identified from the selected health conditions.")
